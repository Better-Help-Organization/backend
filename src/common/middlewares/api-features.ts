...
//   // Clone base query for count BEFORE pagination
//   let countQuery = this.query.clone();

//   // Apply eager relations if needed
//   if (this.target && this.tableName) {
//     this.query = this.applyEagerRelations(this.query, this.target, this.tableName);
//     countQuery = this.applyEagerRelations(countQuery, this.target, this.tableName);
//   }

//   // Get raw data for pagination count (without skip/take)
//   const rawData = await countQuery.getMany();

//   // Apply distinctField deduplication on raw data
//   let dedupedData = rawData;
//   if (this.distinctField) {
//     const fieldParts = this.distinctField.split(".");
//     const getter = (o: any) =>
//       fieldParts.reduce((acc, p) => (acc ? acc[p] : undefined), o);

//     dedupedData = rawData.filter(
//       (obj, index, arr) =>
//         index === arr.findIndex((o) => getter(o) === getter(obj))
//     );
//   }

//   // Now compute correct totals
//   const totalItems = dedupedData.length;
//   const totalPages = Math.ceil(totalItems / (this.parsedLimit || 10));

//   // Finally paginate the deduped dataset in-memory
//   const start = ((this.parsedPage || 1) - 1) * (this.parsedLimit || 10);
//   const end = start + (this.parsedLimit || 10);
//   const pageData = dedupedData.slice(start, end);

//   const pageSize = this.parsedLimit || 10;   // default limit
//   const currentPage = this.parsedPage || 1;  // default page
...
// }
  ...
  //   let currentPage = this.parsedPage
  //   let pageSize = this.parsedLimit

  //   if (this.target && this.tableName) {
  //     this.query = this.applyEagerRelations(this.query, this.target, this.tableName);
  //   }

  //   let data = await this.query.getMany()

  //   let totalItems = await this.query.getCount()
  //   let totalPages = Math.ceil(totalItems / (parseInt(this.queryParams?.take,10) || 10));
  ...
  //   if (fieldParts.length === 1) {
  //       const key = fieldParts[0];
  //       data = data.filter((obj, index, arr) =>
  //         index === arr.findIndex(o => {
  //           // handle nested or undefined safely
  //           const a = o && o[key];
  //           return a === (obj && obj[key]);
  ...
  //     totalPages = Math.ceil(totalItems / (this.parsedLimit || 10));
  //     currentPage = this.parsedPage || 1;  // default page
  //     pageSize = this.parsedLimit || 10;   // default limit
  ...
  //   //   data = dedupedData.slice(start, end);
  //   //   totalItems = dedupedData.length;
  //   //   console.log({totalItems})
  //   //   console.log({totalPages})
  //   //   console.log({currentPage})
  //   //   console.log({pageSize})
